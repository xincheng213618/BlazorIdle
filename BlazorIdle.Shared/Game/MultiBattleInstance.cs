...
namespace BlazorIdle.Game
{
    ...
    public sealed class MultiBattleInstance
    {
        ...
        /// <summary>
        /// Phase 10.3: 检查角色是否有足够资源施放技能
        /// Phase 10.3: Check if character has enough resources to cast skill
        /// </summary>
        /// <param name="characterId">角色ID / Character ID</param>
        /// <param name="skillId">技能ID / Skill ID</param>
        /// <returns>如果资源足够返回 true / True if resources are sufficient</returns>
        public bool CheckSkillResourceCost(string characterId, string skillId)
        {
            var skill = _skillRepository.GetSkill(skillId);
            if (skill == null)
                return false;

            // Get character's BuffOwner for resource checking
            if (!_playerBuffOwners.TryGetValue(characterId, out var buffOwner))
                return false;

            // Create a minimal battle context for resource checking
            var context = new BattleContext
            {
            ...
            return _resourceManager.CheckResourceCost(skill, context);
        }

        /// <summary>
        /// Phase 10.3: 检查技能条件是否满足
        /// Phase 10.3: Check if skill conditions are met
        /// </summary>
        /// <param name="characterId">角色ID / Character ID</param>
        /// <param name="skillId">技能ID / Skill ID</param>
        /// <returns>如果条件满足返回 true / True if conditions are met</returns>
        public bool CheckSkillConditions(string characterId, string skillId)
        {
            var skill = _skillRepository.GetSkill(skillId);
            if (skill == null || skill.Conditions == null)
            ...
            // Create battle context for condition checking
            var context = new BattleContext
            {
            ...
            // Use the existing CheckConditions method
            return _conditionChecker.CheckConditions(skill, context, isCasterPlayer: true, casterId: characterId);
        }
