...
- [x] P3+.2 创建 monsterskills.json 配置文件 ✅
  - ✅ 位置：BlazorIdle.Shared/Config/monsterskills.json
  - ✅ 包含通用怪物攻击技能（monster_attack_basic）
  - ✅ **重要**：怪物技能ID必须以 `monster_` 或 `enemy_` 开头（用于 SkillResolver 识别）

- [x] P3+.3 扩展 monsters.json 配置 ✅
  - ✅ 所有怪物添加 `normalAttackSkillId` 字段
  - ✅ 指向通用技能 `"monster_attack_basic"`
  - ✅ 保留为特殊怪物自定义技能的灵活性

- [x] P3+.4 SkillRepository 双文件加载 ✅
  - ✅ 同时加载 skills.json（玩家技能）
  - ✅ 同时加载 monsterskills.json（怪物技能）
  - ✅ 所有技能在同一仓库中查询

- [x] P3+.5 SkillResolver 怪物技能支持 ✅
  - ✅ 修复伤害计算（识别 `monster_*` 前缀）
  - ✅ 修复 Buff 系统识别
  - ✅ 修复伤害浮动计算
  - ✅ 修复暴击检查

- [x] P3+.6 ExecuteSkill 统一重构 ✅
  - ✅ 统一玩家和怪物技能执行逻辑
  - ✅ 使用 `isCasterPlayer` 标志区分
  - ✅ 消除约 70 行重复代码
...
| **阶段 P3+ - 怪物技能系统基础整合** | ✅ 已完成 | 2-3h | 0 (复用现有) |
...
### 2025-11-17 v3.0
- ✅ 新增阶段 P3+：怪物技能系统基础整合（2-3h，复用现有测试）
...
**最后更新：** 2025-11-17 v3.0  
**维护者：** @copilot  
**状态：** 已更新，怪物技能系统基础整合已完成
