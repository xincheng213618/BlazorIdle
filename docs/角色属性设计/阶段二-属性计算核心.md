...
namespace BlazorIdle.Shared.Game
{
    ...
    public static class AttributeCalculator
    {
        ...
        /// <param name="equipMainStat">装备提供的额外主属性（预留，MVP可传0）</param>
        ...
        {
            // 步骤1：计算主属性总值
            var mainStatTotal = CalculateMainStat(level, config.MainStat, equipMainStat);

            // 步骤2：计算派生属性
            var result = new CalculatedAttributes
            {
                MainStatTotal = mainStatTotal,
            ...
            result.HastePercent = CalculateHaste(
                mainStatTotal,
                config.Weights.HastePercentPerMain,
                config.Weights.HasteCap
            );

            result.CritChancePercent = CalculateCrit(
                config.Baseline.CritBase,
                mainStatTotal,
                config.Weights.CritChancePercentPerMain,
        ...
        /// 计算主属性总值
        /// </summary>
        private static int CalculateMainStat(int level, MainStatConfig mainStatConfig, int equipBonus)
        {
            if (level < 1)
            {
            ...
            // 公式：MainStat = base + perLevel × (level - 1) + equipBonus
            var mainStat = mainStatConfig.Base + mainStatConfig.PerLevel * (level - 1) + equipBonus;
            
            return Math.Max(0, mainStat); // 确保非负
        }
        ...
        /// 计算急速百分比
        /// </summary>
        private static double CalculateHaste(int mainStat, double perMainCoeff, double cap)
        {
            // 公式：Haste% = min(mainStat × perMainCoeff, cap)
            var haste = mainStat * perMainCoeff;
            
            return Math.Clamp(haste, 0.0, cap);
        }

        /// <summary>
        /// 计算暴击率
        /// </summary>
        private static double CalculateCrit(double critBase, int mainStat, double perMainCoeff, double cap)
        {
            // 公式：Crit% = min(critBase + mainStat × perMainCoeff, cap)
            var crit = critBase + mainStat * perMainCoeff;
            
            return Math.Clamp(crit, 0.0, cap);
        }

        /// <summary>
        /// 计算生命值
        /// </summary>
        private static int CalculateHp(int baseHp, int mainStat, double perMainCoeff)
        {
            // 公式：HP = baseHp + mainStat × perMainCoeff
            var hp = baseHp + mainStat * perMainCoeff;
...
namespace BlazorIdle.Tests.Game
{
    public class AttributeCalculatorTests
    {
        ...
        [Fact]
        public void Calculate_Level1_ReturnsCorrectMainStat()
        {
            // Arrange
        ...
        [Fact]
        public void Calculate_Level10_ReturnsCorrectMainStat()
        {
            // Arrange
            var config = GetWarriorConfig();
        ...
        public void Calculate_Level1_ReturnsCorrectHp()
        {
            ...
            // Assert
            // baseHp=200, mainStat=10, coeff=12
            // 200 + 10*12 = 320
            Assert.Equal(320, result.MaxHp);
        }
        ...
        public void Calculate_Level1_ReturnsCorrectCrit()
        {
            ...
            // Assert
            // critBase=0.05, mainStat=10, coeff=0.001
            // 0.05 + 10*0.001 = 0.06 (6%)
            Assert.Equal(0.06, result.CritChancePercent, 3);
        }
