...
```json
{
  ...
  "mage": {
    ...
    "weights": {
      ...
      "hastePercentPerMain": 0.0015,
      "critChancePercentPerMain": 0.0008,
      "maxHpPerMain": 8,
      "critCap": 0.6,
      "hasteCap": 0.4
    }
  ...
  "ranger": {
    ...
    "weights": {
      ...
      "hastePercentPerMain": 0.003,
      "critChancePercentPerMain": 0.0012,
      "maxHpPerMain": 10,
      "critCap": 0.6,
      "hasteCap": 0.4
    }
...
5. **mainStat**: 主属性成长
   - `base`: 1级基础值
   - `perLevel`: 每级增长值
6. **baseline**: 职业基线属性（不依赖主属性的固定值）
   - `baseDamage`: 基础普攻伤害
   - `baseSpecialDamage`: 基础技能伤害
   - `baseAPS`: 基础攻击速度（次/秒）
   - `baseHp`: 基础生命值
   - `baseSpecialIntervalSec`: 技能间隔（秒）
   - `critBase`: 基础暴击率（百分比小数）
   - `variance`: 伤害浮动（百分比小数）
   - `reviveSec`: 复活时间（秒）
   - `critMultiplier`: 暴击倍率（固定2.0）
7. **weights**: 主属性到派生属性的转换权重
   - `damagePerAttackPerMain`: 每点主属性增加的普攻伤害
   - `specialDamagePerMain`: 每点主属性增加的技能伤害
   - `hastePercentPerMain`: 每点主属性增加的急速百分比
   - `critChancePercentPerMain`: 每点主属性增加的暴击率
   - `maxHpPerMain`: 每点主属性增加的最大生命
...
namespace BlazorIdle.Shared.Models
{
    ...
    public class ProfessionAttributeConfig
    {
        ...
        public MainStatConfig MainStat { get; set; } = new();
    ...
    public class AttributeWeights
    {
        ...
        [JsonPropertyName("hastePercentPerMain")]
        public double HastePercentPerMain { get; set; }

        [JsonPropertyName("critChancePercentPerMain")]
        public double CritChancePercentPerMain { get; set; }

        [JsonPropertyName("maxHpPerMain")]
        public double MaxHpPerMain { get; set; }
...
- [ ] 配置文件 `professionAttributes.json` 创建完成且格式正确
- [ ] 数据模型类创建完成且编译通过
- [ ] API端点添加完成且可以正常访问
- [ ] 前端服务创建完成且已注册
- [ ] 配置可以成功从服务器加载到前端
