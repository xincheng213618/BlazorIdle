...
如果引用不存在的ID，`GameConfig.GetMonster()` 会返回 null，
该怪物组会被跳过（通过 `if (monsterDef == null) continue;`），
不会抛出异常，但敌人队伍中不会包含该怪物。建议在测试时检查敌人队伍数量确认配置正确。

### Q: 场景数量有限制吗？

A: 
没有硬性限制，但从可维护性和用户体验角度建议：
- 保持场景数量合理（推荐20-50个）以便于管理和选择
- 过多场景会让下拉菜单过长，影响用户体验
- 使用清晰的命名和描述
- 按难度或类型分组组织
- 如需更多场景，考虑实现分类或筛选功能
